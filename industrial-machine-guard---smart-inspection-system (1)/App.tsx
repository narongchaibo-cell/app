...
const App: React.FC = () => {
  ...
  /**
   * 🛡️ SUPER ROBUST ATOMIC DELETION (v3.0)
   * ปรับปรุงให้ทำงานแบบเบ็ดเสร็จในขั้นตอนเดียวเพื่อแก้ปัญหา Google AI Studio ลบไม่ได้
   */
  const handleMoveRecordToTrash = useCallback((recordId: string) => {
    if (!currentUser) return;
    
    // ตรวจสอบสิทธิ์ที่ฝั่ง Logic เพื่อความปลอดภัยสูงสุด
    const canManageAction = currentUser.role === UserRole.ADMIN || 
    ...
    // ใช้ Functional State Update เพื่อดึงค่าที่เป็นปัจจุบันที่สุดในระบบ (Real-time snapshot)
    setRecords(prev => {
      // ค้นหารายการจาก State ล่าสุดแทนการใช้ Closure
      const recordToMove = prev.find(r => r.id === recordId);
      
      if (!recordToMove) {
        console.warn("Target record not found in current state:", recordId);
        addToast('ล้มเหลว', 'ไม่พบรายการที่ต้องการลบในฐานข้อมูล', 'system');
        return prev;
      }

      const deletedAt = new Date().toISOString();
      const trashItem = { ...recordToMove, deletedAt };

      // คำนวณข้อมูลใหม่
      const nextRecords = prev.filter(r => r.id !== recordId);
      
      // อัปเดต localStorage ทันที
      localStorage.setItem(KEYS.RECORDS, JSON.stringify(nextRecords));
      
      // อัปเดตถังขยะควบคู่กัน
      setDeletedRecords(prevTrash => {
        const nextTrash = [trashItem, ...prevTrash];
        localStorage.setItem(KEYS.TRASH_RECORDS, JSON.stringify(nextTrash));
        // ส่งสัญญาณซิงก์ถังขยะ
        dbChannel.postMessage({ type: 'SYNC_RELOAD', senderId: currentUser.id, key: KEYS.TRASH_RECORDS, payload: nextTrash });
        return nextTrash;
      });

      // ส่งสัญญาณซิงก์ข้อมูลหลัก
      dbChannel.postMessage({ type: 'SYNC_RELOAD', senderId: currentUser.id, key: KEYS.RECORDS, payload: nextRecords });
  ...
  }, [currentUser, KEYS, addNotification, addToast]);
